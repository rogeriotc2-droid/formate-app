...
type PublicSssp = {
  id: number; projectName: string; siteId?: number | null;
  status: string; data: unknown; createdAt: string; updatedAt: string;
  signToken?: string;
};
...
function PcbuTable({ pcbu, fallbackCompany, fallbackRole }: { pcbu?: Pcbu; fallbackCompany?: string; fallbackRole?: string }) {
  ...
  return (
    <table className="kv-table">
      <tbody>{rows.map(([k, v]) => <tr key={k}><td style={{ width: "38%" }}>{k}</td><td>{v}</td></tr>)}</tbody>
    </table>
  );
}

export default function SsspPublicView() {
  const [, params] = useRoute("/sssp/:token");
  const token = params?.token ?? "";
  const [sssp, setSssp] = useState<PublicSssp | null>(null);
  const [loading, setLoading] = useState(true);
  const [notFound, setNotFound] = useState(false);

  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/sssps/view/${encodeURIComponent(token)}`)
      .then(r => { if (!r.ok) { setNotFound(true); setLoading(false); return null; } return r.json(); })
      .then(data => { if (data) { setSssp(data); setLoading(false); } })
      .catch(() => { setNotFound(true); setLoading(false); });
  }, [token]);

  useEffect(() => {
    if (sssp) document.title = `SSSP — ${sssp.projectName}`;
  }, [sssp]);
  ...
  if (loading) return <div style={{ padding: 40, fontFamily: "sans-serif", color: "#666" }}>Loading…</div>;
  if (notFound || !sssp) return (
    <div style={{ padding: 40, fontFamily: "sans-serif" }}>
      <div style={{ color: "#c00", marginBottom: 8, fontWeight: 700 }}>SSSP not found.</div>
      <div style={{ color: "#666", fontSize: 14 }}>This link may have expired or the plan was deleted. Contact the contractor who sent it.</div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="no-print" style={{
        ...
        padding: "10px 20px", display: "flex", alignItems: "center", gap: 12, fontSize: 13, fontFamily: "sans-serif",
      }}>
        <span style={{ fontWeight: 800 }}><span style={{ color: "#fff" }}>FOR</span><span style={{ color: ORANGE }}>MATE</span></span>
      ...
      {/* Sign reminder — screen only, shown until PCBU 1 has signed */}
      {!d.pcbu1SignedBy && sssp.signToken && (
        <div className="no-print" style={{
          background: "#FFF7ED", borderBottom: "1px solid #FED7AA", padding: "12px 20px",
          display: "flex", alignItems: "center", justifyContent: "center", gap: 12, flexWrap: "wrap", fontFamily: "sans-serif",
        }}>
          <span style={{ fontSize: 14, color: "#9A3412", fontWeight: 600 }}>This plan is waiting for your signature.</span>
          <a href={`${import.meta.env.BASE_URL}sign/${sssp.signToken}`}
             style={{ background: ORANGE, color: "#fff", padding: "8px 20px", borderRadius: 6, textDecoration: "none", fontSize: 13, fontWeight: 700 }}>
            Review &amp; Sign →
          </a>
        </div>
      )}
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "28px 32px" }}>
        ...
        <div style={{ minHeight: "85vh", display: "flex", flexDirection: "column", justifyContent: "space-between", paddingBottom: 40 }}>
          <div>
            {d.pcbu2?.logoUrl ? (
              <img src={`/api/storage${d.pcbu2.logoUrl}`} alt={d.pcbu2.company ?? "Company logo"} style={{ maxHeight: 64, maxWidth: 240, objectFit: "contain", marginBottom: 32, display: "block" }} />
            ) : (
              <div style={{ border: `2px dashed ${BORDER}`, borderRadius: 6, padding: "12px 20px", display: "inline-block", marginBottom: 32, color: "#9CA3AF", fontSize: 9 }}>COMPANY LOGO</div>
            )}
            <h1 style={{ fontSize: 26, fontWeight: 900, color: ORANGE, marginBottom: 6 }}>Site-Specific Safety Plan</h1>
            <p style={{ color: "#6B7280", fontSize: 10.5, marginBottom: 32 }}>Prepared in line with the Site Safe NZ SSSP framework (V22019JUNE) · WorkSafe New Zealand</p>
        ...
        <div className="page-break section">
          ...
          <p style={{ marginBottom: 12, color: "#6B7280", fontSize: 9 }}>This agreement establishes the basis on which businesses agree to work together on this site.</p>
          ...
          <PcbuTable pcbu={d.pcbu2} fallbackRole="PCBU 2 — Subcontractor" />
          ...
          <table className="kv-table">
            <tbody>
              ...
              ].map(([k, v]) => <tr key={k}><td>{k}</td><td>{v as string}</td></tr>)}
            </tbody>
          </table>
        </div>
        ...
        {(d.hazards ?? []).length > 0 && (
          <div className="page-break section">
            ...
            <p style={{ marginBottom: 8, fontSize: 9, color: "#6B7280" }}>
              Site: <strong>{d.siteAddress || "—"}</strong>&nbsp;&nbsp;·&nbsp;&nbsp;
              PCBU 1: <strong>{d.pcbu1?.company || "—"}</strong>&nbsp;&nbsp;·&nbsp;&nbsp;
              PCBU 2: <strong>{d.pcbu2?.company || "—"}</strong>
            </p>
            <table>
              <thead>
                <tr>
        ...
        {(d.taskSteps ?? []).length > 0 && (
          <div className="page-break section">
            ...
            {(d.ppeRequired ?? []).length > 0 && (
              <><h3>PPE Required</h3>
                <ul style={{ paddingLeft: 18, fontSize: 9.5, lineHeight: 1.8 }}>
        ...
        {/* SECTION 5 — PPE standalone */}
        {(d.taskSteps ?? []).length === 0 && (d.ppeRequired ?? []).length > 0 && (
          <div className="page-break section">
          ...
          {(d.emergencyContacts ?? []).length > 0 && (
            <><h3>Emergency Contacts</h3>
              <table>
                ...
                <tbody>{(d.emergencyContacts ?? []).map((c, i) => <tr key={i}><td>{c.name}</td><td>{c.role}</td><td>{c.phone}</td></tr>)}</tbody>
              </table></>
          )}
        </div>
        ...
        <div className="section">
          ...
          <table>
            <thead>
              <tr><th>Name</th><th>Role on site</th><th>Key role</th><th>Training / qualifications</th><th>Competence</th></tr>
            </thead>
            <tbody>
              <tr>
          ...
          <div className="page-break section">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
              {d.photos.map((p: { id: string; objectPath: string; caption?: string }) => (
                <div key={p.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 4, overflow: "hidden" }}>
                  <img
        ...
        <div id="signatures" className="page-break section">
          <h2>9. Declaration &amp; Signatures</h2>
          <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
            <div className="sig-block">
              <div style={{ fontWeight: 700, fontSize: 9.5, marginBottom: 8 }}>PCBU 1 (Principal) — {d.pcbu1?.company || "——————————"}</div>
              <table style={{ fontSize: 9 }}>
                <tbody>
                  <tr>
                    <td style={{ width: 100, fontWeight: 600, border: "none", paddingLeft: 0 }}>Signed by:</td>
                    <td style={{ border: "none" }}><div className="sig-line">{d.pcbu1SignedBy || ""}</div></td>
                  </tr>
                  <tr>
                    <td style={{ fontWeight: 600, border: "none", paddingLeft: 0 }}>Signature:</td>
                    <td style={{ border: "none", paddingTop: 4 }}>
                      {d.pcbu1SignatureImage
                        ? <img src={d.pcbu1SignatureImage} alt="Signature" style={{ maxHeight: 60, maxWidth: 200, display: "block" }} />
                        : <>
                            <div style={{ height: 50, borderBottom: `1px solid ${BORDER}`, width: 180 }} />
                            {sssp.signToken && (
                              <div className="no-print" style={{ marginTop: 8 }}>
                                <a href={`${import.meta.env.BASE_URL}sign/${sssp.signToken}`}
                                   style={{ display: "inline-block", background: ORANGE, color: "#fff", fontSize: 10, fontWeight: 700, padding: "6px 14px", borderRadius: 4, textDecoration: "none" }}>
                                  ✍️ Tap here to sign
                                </a>
                              </div>
                            )}
                  ...
                  <tr>
                    ...
                    <td style={{ border: "none" }}>
                      <div className="sig-line">{d.pcbu1SignedDate ? format(new Date(d.pcbu1SignedDate), "d MMMM yyyy") : ""}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="sig-block" style={{ borderColor: ORANGE }}>
              <div style={{ fontWeight: 700, fontSize: 9.5, marginBottom: 8 }}>PCBU 2 (Contractor) — {d.pcbu2?.company || ""}</div>
              <table style={{ fontSize: 9 }}>
                <tbody>
                  <tr>
                    <td style={{ width: 100, fontWeight: 600, border: "none", paddingLeft: 0 }}>Signed by:</td>
                    <td style={{ border: "none" }}><div className="sig-line">{d.pcbu2SignedBy || d.pcbu2?.contact || ""}</div></td>
                  </tr>
                  <tr>
                    <td style={{ fontWeight: 600, border: "none", paddingLeft: 0 }}>Signature:</td>
                    <td style={{ border: "none", paddingTop: 4 }}>
                      {d.pcbu2SignatureImage
                        ? <img src={d.pcbu2SignatureImage} alt="Signature" style={{ maxHeight: 60, maxWidth: 200, display: "block" }} />
                        : <div style={{ height: 50, borderBottom: `1px solid ${BORDER}`, width: 180 }} />
                      }
                    </td>
                  </tr>
                  <tr>
                    <td style={{ fontWeight: 600, border: "none", paddingLeft: 0 }}>Date:</td>
                    <td style={{ border: "none" }}>
                      <div className="sig-line">{d.pcbu2SignedDate ? format(new Date(d.pcbu2SignedDate), "d MMMM yyyy") : ""}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 14, fontSize: 8, color: "#9CA3AF", marginTop: 24 }}>
            <strong><span style={{ color: DARK }}>FOR</span><span style={{ color: ORANGE }}>MATE</span></strong> · Site-Specific Safety Plan · {today} · WorkSafe NZ: 0800 030 040 · Emergency: 111
          </div>
        </div>
      </div>
    </>
